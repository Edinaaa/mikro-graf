      ...
      @auth
            @if(auth()->user()->hasRole('admin'))
                  <div class="grid bg-white mb-4 rounded-lg shadow-xl w-11/12 md:w-9/12 lg:w-1/2">
                        ...
                        <form name="fontupdatefrm" action="{{ route('font.update',$font->id) }}" onsubmit="return validateUpdateForm('fontupdatefrm')" method="post" enctype="multipart/form-data" >
                              <!-- Add CSRF Token -->
                              @csrf
                              <div class="flex justify-center pt-4">
                              <div class="flex">
                                    <h1 class="text-gray-600 font-bold md:text-2xl text-xl">Font</h1>
                              </div>
                              </div>

                              <x-input id="naziv" label="Naziv font" value="{{ $font->naziv}}" class="mt-5 mx-7">
                              @error("naziv")
                              ...
                              <x-input type="checkbox" checked="{{ $font->aktivan}}" id="aktivan[]" label="aktivan" class="mt-5 mx-7" value="{{ $font->aktivan}}"></x-input>
