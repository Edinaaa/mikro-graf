@props(['stanja'=>'$stanja','idnarudzbe'=>$idnarudzbe,'idBO'=>$idBO, 'idMP'=>$idMP, 'idM'=>$idM])
  ...
  <div id="{{$idBO}}" class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div id="{{$idMP}}" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden  shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
      <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 ">
          ...
          <form method="POST" action="{{ route('narudzba.update',$idnarudzbe) }}">
                    @csrf
                ...
                <select id="stanjes_id" name="stanjes_id" autocomplete="stanjes_id" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm">
