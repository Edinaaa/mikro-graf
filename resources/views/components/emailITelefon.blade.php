@props(['idBO'=>$idBO, 'idMP'=>$idMP, 'idM'=>$idM])
    ...
    <div id="{{$idMP}}" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden  shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
      <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 ">
          ...
          <form method="POST" action="{{ route('korpa.store') }}">
                    ...
                    <x-input id="telefon" type="telefon" label="Telefon" value="" class="mt-5 mx-7"></x-input>
  
                    @endguest
                   
                    <div class='flex items-center justify-center  md:gap-8 gap-4 py-5'>
                        <button type="submit" onClick="Hide('{{$idBO}}', '{{$idMP}}', '{{$idM}}')"  class='w-auto bg-primary-600 hover:bg-primary-700 rounded-lg shadow-xl font-medium text-white px-4 py-2'>Naruci</button>
                    </div>
                </form>
